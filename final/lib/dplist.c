...
#define DPLIST_INSTANCE_NOT_IN_LIST -1  // element/reference not found in list

#ifdef DEBUG
	#define DPLIST_DEBUG_PRINTF(...) 									                        \
		do {											                                        \
...
#else
	#define DPLIST_DEBUG_PRINTF(...) (void)0
#endif


#define DPLIST_ERR_HANDLER(condition, err_code)                                                         \
	do {						                                                                        \
        if ((condition)) DPLIST_DEBUG_PRINTF(#condition " failed, error condition: " #err_code "\n");   \
        assert(!(condition));                                                                           \
    } while(0);
...
dplist_t * dpl_insert_at_index(dplist_t * list, void * element, int index, bool insert_copy) {
    ...
    dplist_node_t *node_at_index;
    if(size > 0) node_at_index = dpl_get_reference_at_index(list, index);
...
dplist_t * dpl_remove_at_index( dplist_t * list, int index, bool free_element) {
    ...
    DPLIST_ERR_HANDLER(node_at_index == NULL, DPLIST_INVALID_ERROR);
...
int dpl_get_index_of_element( dplist_t * list, void * element ) {
    ...
    if(list->head == NULL) return DPLIST_INSTANCE_NOT_IN_LIST;
    for(index = 0; list->element_compare(node->element, element) != 0; node = node->next, index++)
        if(node->next == NULL) return DPLIST_INSTANCE_NOT_IN_LIST;
...
int dpl_get_index_of_reference( dplist_t * list, dplist_node_t * reference ) {
    ...
    return DPLIST_INSTANCE_NOT_IN_LIST;
}
...
dplist_t * dpl_insert_at_reference( dplist_t * list, void * element, dplist_node_t * reference, bool insert_copy ) {
    ...
    if(index == DPLIST_INSTANCE_NOT_IN_LIST) return list;
